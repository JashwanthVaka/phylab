...
// 3. A deployed image must not carry personally-owned sources. The Dockerfile does
...
// 4. npm ci refuses to run when the lock file and manifest disagree, which would
...
// 5. contentIndex feeds /api/content/index and the static build, so it must not
