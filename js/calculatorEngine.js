...
export function renderCalculator(selected='SUVAT') { const tool=tools.find(item=>item.name===selected)||tools[0]; return `<article class="calculator-card"><div class="calculator-head"><span class="tag">PHYSICS CALCULATOR</span><select data-calculator-select aria-label="Choose a calculator">${tools.map(item=>`<option ${item.name===tool.name?'selected':''}>${escapeHTML(item.name)}</option>`).join('')}</select></div><code>${escapeHTML(tool.formula)}</code><form data-calculator-form data-calculator-id="${tool.id}">${tool.fields.map(([key,label,value])=>`<label>${escapeHTML(label)}<input required type="number" step="any" name="${escapeHTML(key)}" value="${value}"></label>`).join('')}<button class="button">Calculate</button></form><output class="calculator-result" aria-live="polite">Enter values and calculate.</output></article>`; }
