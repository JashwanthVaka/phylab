...
export function bindAI() {
  ...
  const renderMessages = () => { messages.innerHTML = history.length ? history.map(messageHTML).join('') : '<p><b>PHY:</b> Choose a teaching mode and ask about the physics you are studying.</p>'; modeInput.value = selectedMode; scrollLatest(); };
  const setPending = value => { send.disabled = value; input.disabled = value; imageInput.disabled = value; stop.hidden = !value; };
  const showError = text => { messages.insertAdjacentHTML('beforeend', `<article class="content-card" role="alert"><h3>PHY needs another try</h3><p>${escapeHTML(text)}</p><button type="button" data-ai-retry>Retry</button></article>`); scrollLatest(); };
  ...
  const ensureConversation = async () => {
    ...
    const row = guestMode ? guestConversationService.create('New PHY conversation') : (await aiHistoryService.createConversation({ title: 'New PHY conversation', context }))?.data;
    if (!row) return false;
  ...
  const readImage = file => new Promise((resolve, reject) => {
    ...
    const reader = new FileReader(); reader.onload = () => resolve(reader.result); reader.onerror = () => reject(new Error('PHY could not read that image.')); reader.readAsDataURL(file);
  });
  const request = async ({ text, reuseUser = false } = {}) => {
    ...
    if (streamError || !assistant.content.trim()) { history = history.filter(item => item !== assistant); renderMessages(); showError(streamError || 'PHY returned an empty response. Please retry.'); return; }
