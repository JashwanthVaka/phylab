...
export const section = (title, body, options = {}) => `<section class="lesson-section" id="${options.id || ''}"><div class="section-title"><p class="eyebrow">${escapeHTML(options.eyebrow || 'KINETIQ')}</p><h2>${escapeHTML(title)}</h2></div>${body}</section>`;
