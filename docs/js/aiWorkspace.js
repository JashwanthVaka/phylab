...
function providerNotice(status) {
  ...
  if (status.static) {
    return '<p class="ai-status">This is the static build of KINETIQ, which runs without a server, so KIT is switched off here. Everything else — lessons, the course library, cases, question patterns, graphs, simulations and quizzes — works exactly as it does in the full app. Run KINETIQ with <code>npm start</code> and an AI key to use the tutor.</p>';
  }
