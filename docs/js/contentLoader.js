...
export class ContentLoader {
  ...
  getIndex() { this.indexPromise ||= this.request('./api/content/index.json'); return this.indexPromise; }
  async getLesson(slug) {
    const key = slugify(slug); if (!this.lessonCache.has(key)) this.lessonCache.set(key, this.request(`./api/content/lessons/${encodeURIComponent(key)}.json`));
